...
Camera::Camera()
{
	this ->cameraPosition = glm::vec3(0.0f, 0.0f, 450.0f);
	this ->cameraViewDirection = glm::vec3(0.0f, 0.0f, -1.0f);
	this ->cameraUp = glm::vec3(0.0f, 1.0f, 0.0f);
	this->cameraRight = glm::cross(cameraViewDirection, cameraUp);
	this->rotationOx = 0.0f;
...
void Camera::rotateOz(float angle)			// added for the ability to "roll underwater"
{
	cameraUp = glm::normalize(glm::vec3((glm::rotate(glm::mat4(1.0f), angle, cameraViewDirection) * glm::vec4(cameraUp, 1))));
	cameraRight = glm::normalize(glm::cross(cameraViewDirection, cameraUp));
	cameraViewDirection = glm::cross(cameraUp, cameraRight);
